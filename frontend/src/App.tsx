...
import { Activity, ArrowDown, ArrowRight, Box, Check, ChevronRight, Circle, Code2, Database, FileJson, GitBranch, Hexagon, Layers3, Loader2, LockKeyhole, Play, Plus, PanelLeft, Radio, RefreshCw, Settings2, ShieldCheck, Square, Terminal, Workflow, X } from 'lucide-react';
...
export default function App() {
  ...
  const navigation = [{name: 'Overview', label: 'Overview', icon: Layers3}, {name: 'Workflows', label: 'Workflows', icon: Workflow}, {name: 'Shared state', label: 'Resources', icon: Database}, {name: 'Event stream', label: 'Activity', icon: Activity}];
  ...
  return <div className={`app-shell ${sidebarHidden ? 'sidebar-hidden' : ''}`}>
    ...
    <div className="main-shell"><header className="topbar"><div><button className="icon-button sidebar-toggle" aria-label={sidebarHidden ? 'Show sidebar' : 'Hide sidebar'} aria-expanded={!sidebarHidden} onClick={() => setSidebarHidden(!sidebarHidden)}><PanelLeft size={22}/></button><strong>{view === 'Shared state' ? 'Resources' : view === 'Event stream' ? 'Activity' : view}</strong></div><div><span className="environment"><span className={online ? 'live-dot' : 'live-dot offline'}/>{online ? 'Connected' : 'Connecting…'}</span><button className="icon-button" aria-label="Connection settings" onClick={() => setModal('connection')}><Settings2 size={20}/></button><button aria-label="New workflow" className="toolbar-create" onClick={() => setModal('workflow')}><Plus size={19}/><span>New workflow</span></button></div></header><main id="main-content" tabIndex={-1}>
      <div className="page-heading"><div><div className="eyebrow">YOUR WORKSPACE, IN SYNC</div><h1>{view === 'Overview' ? 'Your agents, in harmony.' : view === 'Shared state' ? 'Resources' : view === 'Event stream' ? 'Activity' : view}</h1><p>{view === 'Overview' ? 'A little room to explore. A shared state you can trust.' : view === 'Workflows' ? 'Every execution, from the first proposal to the final commit.' : view === 'Shared state' ? 'Authoritative data and schema versions for every agent.' : 'A durable record of the decisions behind your state.'}</p></div><div className="heading-note"><span className="note-orbit"><Hexagon size={22}/></span><span>Independent minds.<br/><strong>One shared world.</strong></span></div></div>
